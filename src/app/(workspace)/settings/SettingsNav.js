      ...
      {sections.map((section) => {
        ...
        return (
          <button
            ...
            className={cn(
              "relative grid w-full grid-cols-[20px_minmax(0,1fr)] items-center gap-3 rounded-l-[1px] rounded-r-[4px] px-3 py-2.5 text-left transition-none",
              active ? "bg-[var(--fx-surface-selected)]" : "hover:bg-[var(--fx-surface-hover)]",
            )}
          >
            {active ? <span aria-hidden="true" className="absolute inset-y-0 left-0 w-[3px] rounded-[2px] bg-[var(--fx-primary)]" /> : null}
            <Icon
