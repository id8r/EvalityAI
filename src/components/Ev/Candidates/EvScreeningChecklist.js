...
/*
  ...
  (`checked` = array of question ids, `onToggle(id)`). No other actions — just a tab to keep on the conversation.
*/
function EvScreeningChecklist({ questions = [], checked = [], onToggle, title = "Pre-Screening Questions to ask" }) {
              ...
              >
                <span
                  className={cn(
                    ...
                    isChecked
                      ? "border-[var(--fx-primary)] bg-[var(--fx-primary)] text-[var(--fx-primary-foreground)]"
                      : "border-[var(--fx-border-strong)] bg-[var(--fx-surface)] group-hover:border-[var(--fx-primary)]",
                  )}
                >
                  {isChecked ? <Check className="size-3" strokeWidth={3} /> : null}
                </span>
                <span className={cn("text-[14px] leading-[20px]", isChecked ? "text-[var(--fx-text-muted)] line-through" : "text-[var(--fx-text)]")}>
                  {item.question}
                </span>
              </button>
            </li>
