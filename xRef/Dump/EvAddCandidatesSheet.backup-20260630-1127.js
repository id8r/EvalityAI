...
function EvAddCandidatesSheet({ open, onOpenChange, mode = "add", job, candidates = [], onPick, onUpload }) {
  ...
  // Same body for both entry points — the age filter applies regardless of mode (only the landing tab + title differ).
          ...
          <div className={cn("grid min-h-0 flex-1 gap-3", showListPane ? "lg:grid-cols-[300px_1px_minmax(0,1fr)]" : "lg:grid-cols-1")}>
            ...
            {showListPane ? (
              <div className="flex min-h-0 flex-col overflow-hidden rounded-[12px] border border-[var(--fx-border)] bg-[var(--fx-bg-soft)]">
                <div className="border-b border-[color:color-mix(in_srgb,var(--fx-border)_72%,transparent)] p-2">
                  {/* Identical in both entry points — age filter + search. */}
                ...
                <div className="min-h-0 flex-1 overflow-y-auto p-2">
                  {visible.length ? (
                    ...
                    <div className="flex h-full flex-col items-center justify-center gap-1 px-4 py-10 text-center">
                      <p className="text-[13px] font-medium text-[var(--fx-text)]">No matches</p>
                      <p className="text-[12px] text-[var(--fx-text-muted)]">Adjust the filter or clear the search.</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex min-h-0 flex-col gap-3 overflow-y-auto">
              {selected ? (
                ...
                <div className="flex flex-1 items-center justify-center rounded-[12px] border border-[var(--fx-border)] bg-[var(--fx-surface)] p-10 text-center">
                  <div className="space-y-2">
                    <FileText className="mx-auto size-6 text-[var(--fx-text-muted)]" />
                    <p className="text-[14px] font-medium text-[var(--fx-text)]">No candidate selected</p>
                    <p className="text-[13px] text-[var(--fx-text-muted)]">No matches for the current filter and search.</p>
                  </div>
                </div>
              )}
            </div>
